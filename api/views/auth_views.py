...
def _secure_cookie(request) -> bool:
    """Honor production cookie settings behind reverse proxies."""
    return bool(getattr(django_settings, "SESSION_COOKIE_SECURE", False) or request.is_secure())


def _cookie_samesite() -> str:
    return getattr(django_settings, "SESSION_COOKIE_SAMESITE", "Lax") or "Lax"
...
def login(request):
    ...
    try:
        ...
        log_audit_action(user, 'login', 'user', user.id, {'ip': ip_address})

        # Send Premium Login Alert Email
...
def logout(request):
    ...
    response.delete_cookie("refresh_token", samesite=_cookie_samesite())
    return response
